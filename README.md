...
Бот:
- проводит онбординг (`/start` -> запрос города -> автоопределение timezone -> кнопки управления),
...
- умеет выдавать ведический день на сегодня (`/today`) по городу пользователя с интерпретацией через OpenRouter,
...
- `/start` — запросить город, автоматически определить таймзону и показать кнопки управления
- `/settings` — текущее расписание и кнопки управления
- `/stop` — выключить рассылку
- `/resume` — включить рассылку
- `/setcity` — запросить и сохранить город (таймзона определяется автоматически)
- `/setlocation` — legacy-алиас к `/setcity`
...
5. Проверь `/health`, потом `/start`, `/setcity`, `/today`, `/refresh`, `/tomorrow`.
...
1. В Telegram отправь `/setcity`.
2. Введи город, например `Moscow` или `Москва`.
